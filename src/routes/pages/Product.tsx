...
import { fetchKakaoAddressByCoords, loadKakaoMapSdk } from '../../components/Map/kakao';
...
export default function Product() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const result = await fetchKakaoAddressByCoords(data.latitude, data.longitude);
  ...
  const handleStatusSelect = async (status: string) => {
    ...
    try {
      ...
      const res = await fetch(`${backUrl}/board/${id}/status?${params.toString()}`, {
        method: 'PATCH',
  ...
  return (
    <div>
        ...
        >
          {images.map((src, index) => (
            <div key={index} className={styles.slide}>
              <img src={src} alt={`상품 이미지 ${index + 1}`} draggable={false} />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className={styles.infoContainer}>
        <p className={styles.category}>{categoryLabelMap[data.category] ?? data.category}</p>
        ...
        <div className={styles.infoRows}>
          <div className={styles.infoRow}>
            ...
            <span className={`${styles.infoValue} ${styles.infoValueUnderline}`}>
      ...
      <div className="button-space">
        ...
        >
          {liked ? <LikeRedHeartIcon width={32} height={32} /> : <LikeGrayHeartIcon width={32} height={32} />}
        </button>
        <div className={styles.chatButtonWrap}>
          {data.owner ? (
            <Button variant="primary" type="button" onClick={() => setStatusDrawerOpen(true)}>
              변경하기
            </Button>
          ) : (
            <Button variant="primary" type="button" onClick={() => navigate('/chat')}>
              채팅하기
            </Button>
          )}
        </div>
