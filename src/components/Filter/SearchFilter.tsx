...
function buildChips(
  f: FilterValue,
  patch: (p: Partial<FilterValue>) => void
): Chip[] {
  const chips: Chip[] = [];
  ...
  if (f.gender)
    chips.push({
      id: 'gender',
      label: f.gender,
      onRemove: () => patch({ gender: '' }),
    });

  if (f.priceMin || f.priceMax) {
    const label = [
      f.priceMin && `${f.priceMin}원`,
      f.priceMax && `${f.priceMax}원`,
    ]
      .filter(Boolean)
      .join(' ~ ');
    chips.push({
  ...
  f.sizes.forEach((s) =>
    chips.push({
      id: `size_${s}`,
      label: s,
      onRemove: () => patch({ sizes: f.sizes.filter((x) => x !== s) }),
    })
  );
  ...
  if (f.dateStart || f.dateEnd) {
    ...
    chips.push({
      id: 'date',
      label,
      onRemove: () => patch({ dateStart: '', dateEnd: '' }),
    });
  }
...
export default function SearchFilter({ value, onChange }: Props) {
  ...
  const updateFilter = (patch: Partial<FilterValue>) =>
