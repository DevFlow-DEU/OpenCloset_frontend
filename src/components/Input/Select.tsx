...
export default function Select({
  ...
  const drawerProps = {
    ...
    onSelect: (val: string) => {
      ...
      registerOnChange({
        target: { value: val, name: restRegister.name },
  ...
  return (
    <div className={styles.inputSelect}>
      ...
      >
        <span
          className={`${styles.inputSelectValue}${selectedValue ? ` ${styles.selected}` : ''}`}
        >
          {selectedValue || placeholder || '선택해 주세요'}
        </span>
        <SlArrowLeft
          className={styles.inputSelectArrow}
          style={{ transform: 'rotate(-90deg)' }}
        />
      </button>
      {isDrawerOpen && drawer === 'size' && <Size {...drawerProps} />}
      {isDrawerOpen && drawer === 'category' && (
        <List type="category" {...drawerProps} />
      )}
      {error && (
